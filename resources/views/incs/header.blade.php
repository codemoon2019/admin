...
<head>
    ...
    <!--favicon-->
    <!--<link rel="icon" href="{{ config('app.url') }}images/favicon-32x32.png" type="image/png"/>-->
		...
		<header class="top-header">
			<nav class="navbar navbar-expand">
				<div class="left-topbar d-flex align-items-center">
					...
					<div class="">
						<img src="{{ config('app.url') }}images/logo.png" class="logo-icon" alt="">
					</div>
				</div>
							...
							<a class="nav-link dropdown-toggle dropdown-toggle-nocaret" href="javaScript:;" data-toggle="dropdown">
								<div class="media user-box align-items-center">
									<div class="media-body user-info">
										...
										<p class="designattion mb-0">{{ auth()->user()->userType->description }}</p>
									</div>
									<img src="{{ auth()->user()->profile_picture_url == '' ? 'http://ssl.gstatic.com/accounts/ui/avatar_2x.png' : config('app.url').auth()->user()->profile_picture_url }} " class="user-img avatar" alt="user avatar">
								</div>
							</a>
		...
		<div class="page-wrapper">
			...
			<div class="sidebar-wrapper" data-simplebar="true">
				<div class="sidebar-header">
					...
					<div class="">
						<img src="{{ config('app.url') }}images/logo.png" class="logo-icon-2" alt="" />
					</div>
				</div>
				...
				@if(auth()->user()->user_type == 2)
					...
					<ul class="metismenu" id="menu">
						...
						<li>
							<a href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form-driver').submit();">
								<div class="parent-icon"><i class="bx bx-power-off"></i>
								</div>
								<div class="menu-title">Sign Out</div>
							</a>
							<form id="logout-form-driver" action="{{ route('logout') }}" method="POST" style="display: none;">
								{{ csrf_field() }}
							</form>
						</li>
					</ul>
